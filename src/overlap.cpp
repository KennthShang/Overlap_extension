...
unordered_map<uint, string> read_sam(const char* sam_file, unordered_map<string, uint>& reads_map, vector<string>& readsData){
    ...
    if(f.is_open()){
        while(getline(f, line)){
            ...
            vector<string> lmap = split(line, '\t');                     // split each part
            if(stoi(lmap[1]) != 4){                                      // if mapped
                if(reads_map.find(lmap[0])!=reads_map.end()){            // if the reads can be found
                    uint idx = reads_map[lmap[0]];                       // get the id of reads
                    result[idx] = readsData[idx];                        // get the reads
                }
...
void submit_job(uint start, uint end, unordered_map<uint, string>& seeds, vector<string>& bwt, vector<string>& rev_bwt, vector<string>& alphabet, uint cutoff, \
    ...
    for(int i = start; i < end; i++){
        ...
        find_all_overlap(seeds, bwt[i], seq_len, rev_bwt[i], alphabet[i], cutoff, C[i], Occ[i], rev_Occ[i], seq_index_array[i], rev_seq_index_array[i], saved_reads, results, r);
    }
        ...
        for (int i=1;i<argc;i++){
            if (i+1 <argc){
                if (strcmp(argv[i],"-S")==0){
                    ...
                    char* thread_tmp = argv[i+1];
                    thread_num = atoi(thread_tmp);
                    i++;
                }
